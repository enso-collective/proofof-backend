...
export const lensWebhook = functions.https.onRequest(async (req, res) => {
  try {
    ...
    lensComment(data.publicationId, `@${handle} your Proof:of ${brandName} afk Brussels is minted! View the attestation: https://www.onceupon.gg/${hash} and view the leaderboard & gallery: https://proofof.bot/events/afk`);
  }  catch(error) {
    console.log(error);
  }
  res.sendStatus(200);
