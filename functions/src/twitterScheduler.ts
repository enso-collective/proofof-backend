    ...
    try {
        ...
        if (newestMentionId !== lastLukosMentionId) {
            ...
            hashtagMentions.data?.forEach(async element => {
                ...
                const hash = await eas_mint(user?.username!, wallet, tweetUrl, photo.url!, element.text, questId);
                await userClient.tweets.createTweet({ text: `@${user?.username} your ${brandName} Proof is created! View your attestations at proofof.bot/profile`, reply: { in_reply_to_tweet_id: element.id } }); 
            });
        }
    } catch(error) {
        console.log(error);
